//
//  FormViewModelImplTests.swift
//  HeavyCrayon
//
//  Created by Juan Felipe Alvarez Saldarriaga on 7/30/16.
//  Copyright © 2016 Juan Felipe Alvarez Saldarriaga. All rights reserved.
//

import XCTest
@testable import HeavyCrayon

class FormViewModelImplTests: XCTestCase {
  
  override func setUp() {
    super.setUp()
  }
  
  override func tearDown() {
    super.tearDown()
  }
  
  func testSaveData() {
    class MockFormRepositoryImpl: FormRepositoryImpl {
      var saveWasCalled = false
      
      override private func save(firstName firstName: String, lastName: String, questionOne: Bool, questionTwo: Bool, questionThree: Bool, questionFour: Bool) throws -> Form {
        saveWasCalled = true
        
        return try! super.save(firstName: firstName,
          lastName: lastName,
          questionOne: questionOne,
          questionTwo: questionTwo,
          questionThree: questionThree,
          questionFour: questionFour)
      }
    }
    
    let context = MockNSManagedObjectContext.setUpInMemoryManagedObjectContext()
    let repository = MockFormRepositoryImpl(context: context)
    let viewModel = FormViewModelImpl(repository: repository)
    
    viewModel.saveData(firstName: "Juan",
      lastName: "Alvarez",
      questionOne: true,
      questionTwo: true,
      questionThree: true,
      questionFour: false)
    
    XCTAssertTrue(repository.saveWasCalled)
  }
}
